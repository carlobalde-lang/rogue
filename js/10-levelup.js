...
// Reused probe offsets (module-level so corner deflection doesn't allocate)
const _DEFL_OFFS = [Math.PI * 0.5, -Math.PI * 0.5, 0.7, -0.7, 1.0, -1.0, Math.PI * 0.25, -Math.PI * 0.25];
function deflectSteering(e, vx, vy, hitR) {
  ...
  for (let k = 0; k < _DEFL_OFFS.length; k++) {
    const a = baseA + _DEFL_OFFS[k];
    const tx = e.x + Math.cos(a) * look;
    const ty = e.y + Math.sin(a) * look;
    if (circleBlocked(tx, ty, hitR)) continue;
    if (!hasFallback) { hasFallback = true; fallbackA = a; }
    const f2 = flowDirectionAt(tx, ty);
    const cost = (f2 && f2.cost < MAX_COST) ? f2.cost : Infinity;
    if (cost < bestCost) { bestCost = cost; bestA = a; }
  }
  if (bestCost < Infinity) return { dx: Math.cos(bestA), dy: Math.sin(bestA) };
  if (hasFallback) return { dx: Math.cos(fallbackA), dy: Math.sin(fallbackA) };
  return { dx: vx, dy: vy };
}
