...
function fireWeapons() {
  ...
  for (const w of p.weapons) {
    ...
    // Overload: weapons with a cooldown may fire a burst right now and then
    // suffer a doubled reload until the stretch window expires. A pending
    // burst fires immediately on the next frames regardless of the timer.
    const wasPending = !!w.burstPending;
    if (!w.burstPending) {
      const stretching = now < (w.overloadUntil || 0);
      const rateNow = (stats.rate > 0 && stretching) ? stats.rate * 2 : (stats.rate || 0);
      if (now - w.lastFired < rateNow) continue;
      if (p.overload > 0 && stats.rate > 0 && !stretching && Math.random() < p.overload * 0.25) {
        w.burstPending = true;
        w.overloadUntil = now + stats.rate;
      }
    }
    ...
    if (wasPending) w.burstPending = false;   // the burst shot already fired this pass
  }
}
